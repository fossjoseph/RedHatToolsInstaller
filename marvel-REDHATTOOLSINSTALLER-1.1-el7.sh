...
echo "
                           ...
                           * RHEL 7.X Base install or Server with UI
...
#------------------------------
  ...
  yum-config-manager --enable epel
  sleep 5
  yum install -y screen yum-utils vim gcc gcc-c++ git rh-nodejs8-npm make automake kernel-devel ruby-devel libvirt-client bind dhcp tftp syslinux* xorg-x11-server-Xorg  xorg-x11-server-common ntp chrony sos crontabs cronie-anacron cronie
  sleep 5
  echo ""
  echo "*********************************************************"
...
function CENTOS7HOSTCOLLECTION {
  ...
  echo " "
  INPUT=${INPUT:-$CENTOS7DEFAULTVALUE}
  if  [ "$INPUT" = "y" -o "$INPUT" = "Y" ] ;the
  echo " "
  echo "***********************************"
  echo "Create a host collection for CentOS 7:"
  echo "***********************************"
  then hammer host-collection create --name='CentOS 7 x86_64' --organization $ORG
    echo " "
    sleep 2
  fi
}
